...
decide() is now async; all tests that call it are marked asyncio accordingly.
"""
...
@pytest.mark.asyncio
async def test_r1_pending_tool_calls_routes_to_tool_executor():
    state = _make_state(pending_tool_calls=[{"id": "c1", "function": {"name": "read_file", "arguments": "{}"}}])
    decision = await _SUP.decide(state)
    assert decision.next_node == "tool_executor"
    assert decision.rule == "R1"


@pytest.mark.asyncio
async def test_r1_empty_pending_tool_calls_falls_through():
...
@pytest.mark.asyncio
async def test_r15_after_tool_executor_routes_to_originator():
...
@pytest.mark.asyncio
async def test_r15_invalid_originator_falls_through():
...
@pytest.mark.asyncio
async def test_r15_no_history_does_not_fire():
...
@pytest.mark.asyncio
async def test_r2a_dispatches_to_in_progress_step_owner():
...
@pytest.mark.asyncio
async def test_r2a_invalid_owner_role_falls_through_to_r2b():
...
@pytest.mark.asyncio
async def test_r2b_routes_to_planner_when_pending_steps_remain():
...
@pytest.mark.asyncio
async def test_r3_routes_to_review_critic_when_evaluator_not_run():
...
@pytest.mark.asyncio
async def test_r3_routes_to_check_convergence_when_evaluator_ran():
...
@pytest.mark.asyncio
async def test_r3_includes_blocked_steps_in_metadata():
...
@pytest.mark.asyncio
async def test_r3_evaluator_ran_in_different_iter_does_not_count():
...
@pytest.mark.asyncio
async def test_r4_dispatches_chain_index_role_when_no_plan():
...
@pytest.mark.asyncio
async def test_r4_advances_with_chain_index():
...
@pytest.mark.asyncio
async def test_r4_invalid_chain_role_falls_through_to_r5():
...
@pytest.mark.asyncio
async def test_r5_routes_to_review_critic_when_evaluator_not_run():
...
@pytest.mark.asyncio
async def test_r5_routes_to_check_convergence_when_evaluator_ran():
...
@pytest.mark.asyncio
async def test_r5_empty_chain_falls_through():
...
@pytest.mark.asyncio
async def test_r1_preempts_plan_rules():
...
@pytest.mark.asyncio
async def test_r15_preempts_plan_rules():
...
@pytest.mark.asyncio
async def test_supervisor_anchor_appends_decision_to_state():
...
@pytest.mark.asyncio
async def test_supervisor_anchor_accumulates_decisions():
...
@pytest.mark.asyncio
async def test_react_loop_under_plan():
    """
    ...
    d1 = await _SUP.decide(state_with_tool)
    ...
    d2 = await _SUP.decide(state_after_tool)
    ...
    d3 = await _SUP.decide(state_coder_done)
...
@pytest.mark.asyncio
async def test_plan_all_done_and_evaluator_ran_routes_to_convergence():
